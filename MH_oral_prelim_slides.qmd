...
* Related research assistantship projects at InTrans.
...
driveplot(
  ...
  x = time_cst, 
  ys = c(speed_mph, gyro_heading_raw, gps_pdop), 
  xlabel = "Time", 
  ylabels = c("Speed (MPH)", "Gyro Heading", "GPS PDOP"), showlegend = TRUE, 
  legendtitle = "Minute", 
  spacing = 0.05)
```
...
* Longitudinal Research on Aging Drivers (LongROAD) [@liLongitudinalResearchAging2017].
  ...
  - Goal: understand unique factors affecting older drivers, including age-related functional decline and self-regulatory driving behaviors.
...
## Relevant Research Assistantship Projects at InTrans

* Developed an R Shiny app to calculate work zone performance metrics.

  - Users upload work zone data in standard formats.
  
  - Place poorly defined work zone locations on the appropriate road.
  
  - Use crash and probe vehicle data (and optionally connected vehicle data) to calculate performance metrics (e.g., number of crashes, average delay, etc.).
  
  - Download reports for overall metrics and metrics for individual projects.
  
* Applying lessons learned from developing the Shiny app to the work zone performance reporting process for Iowa.

## Example Use Cases for Vehicle Trajectory Data

Necessary level of GPS precision depends on the use case.

* Only need road usage information to
...
Goal: associate (erroneously) recorded locations of a moving vehicle to the road system.
...
* @greenfeldMatchingGPSObservations2002 noted that the projection onto the road system may not correspond to the vehicle's true location.

## Weighted Average

:::{.column width="50%"}
* @quddusGeneralMapMatching2003: weighted average of the orthogonal projection point and another point derived from the vehicle speed and the bearing of the selected road.

* This method was also used in @ochiengMapmatchingComplexUrban2003 and @quddusHighAccuracyFuzzy2006.

:::

:::{.column width="50%"}
![Example of Quddus, Noland, and Ochieng's map-matching algorithm with position determination by the  weighted average method, as implemented by @gorteFuzzyMMMapMatching2015. Original points are blue and map-matched points are red.](weighted_average_ex.png)

:::

## Kalman Filter

* @kimEfficientUseDigital2000: Kalman filter corrects bias in initial orthogonal projection estimates of vehicle locations using time and measurement updates.

![Example of the Kalman filter method from @kimEfficientUseDigital2000.](kim_kf_ex.png)
...
## Key Contributions to Workflows with Large-Scale Spatiotemporal Transportation Data

* **Visualize** and explore the data with `DrivePlotR`.

* **Process** the data by improving estimates of vehicle locations on the road system.

* **Analyze** the processed data to draw new insights.
