...
* Instrumentation installed in participants' vehicles monitors their driving.
...
* Motivation: 
  ...
  - Turning across traffic is a challenging maneuver for older drivers, but avoidance is rarely self-reported [@molnarRelationshipSelfRegulationDrivingRelated2008; @molnarSelfregulationDrivingOlder2013; @beckExaminingPatternsDriving2022].
...
* Goal: quantify the relationship between the probability of an older driver's cognitive status (aging normally or cognitively impaired) and their odds of making a left turn.

## Simple Turn Detection Method

Gyroscopic heading changes of $\pm 10^{\circ}$ or more in 1 sec were classified as turns (developed based on `DrivePlotR` visualizations).
...
* Caution: noisy signal, low number of participants, simple turn detection method, and simple model.
...
* Observations taken regularly (e.g., every 1-3 sec. for our data sources).
