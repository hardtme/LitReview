...
\mh{The organization of this work is as follows.} We first discuss some essential background regarding GPS data in @sec-crs, @sec-gps, and @sec-dop. We describe NDS data in @sec-nds and also describe CV data in @sec-cv. \mh{@sec-problemoverview provides an overview of the map-matching problem. Some map-matching algorithms and their drawbacks are discussed in @sec-MMalgos. Current solutions to the position determination problem in map-matching are provided in @sec-roadselection. A proposal for this work is found in @sec-proposal.}
