...
\mh{The simplest way to solve the road selection problem is to find the road segment closest to the recorded location of the vehicle. @bernsteinIntroductionMapMatching1996 found the "node" or This method was used in several early works on map-matching, including @bernsteinIntroductionMapMatching1996, @whiteMapMatchingAlgorithms2000, and @taylorRoadReductionFiltering2001. The position determination problem is subsequently solved by orthogonally projecting the recorded vehicle location onto the closest road segment.}
...
@fig-naive-map-matching shows a shortcoming of \mh{the nearest road method} using the same example as @fig-gps-drift. Once again, the original points are blue and the matched points are red. The orthogonal projection looks good for six of the ten points, but the four points west of the intersection are problematic. The original points may be closer to the east-west road segment instead of the north-south road segment, but the match does not fit with the rest of the vehicle trajectory. \mh{A more sophisticated map-matching method is needed.}
