...
@isikIntegrityAnalysisGPSBased2020 noted that there is no agreed-upon standard for relating DOP coefficient values to the quality of navigation data. Recall that lower DOP coefficient values should indicate higher quality navigation data. Isik et al. presented the scale for DOP coefficient values shown in @tbl-dop based on the literature and the results of their own work.
...
1. On which road is the vehicle driving?

2. Where on the road is the vehicle driving?
...
There are many map-matching algorithms available, ranging from simple to complex. A simple way to solve the GPS drift problem is to find the road segment closest to the recorded vehicle location and then orthogonally project the vehicle location onto the closest road segment. \mh{InTrans uses this strategy to associate CV data points to the closest roadway, although the priority of a roadway for association to the CV data points is determined by the roadway's facility type. For example, a point is more likely to be matched to a nearby interstate highway than a nearby local road.} @sakiPracticalGuideOpenSource2022 noted that the method of orthogonal projection to the nearest road was used by @bernsteinIntroductionMapMatching1996, @whiteMapMatchingAlgorithms2000, and @taylorRoadReductionFiltering2001. This method is fast, but Saki et al. commented that it is sensitive to outliers. 
...
@fig-mmarrows shows the same map-matching result as @fig-naive-map-matching, but the original points are shown with underlying arrows that indicate the vehicle heading. @fig-mmcircarrows provides another version of the plot, this time with white arrows inside the blue points representing the original GPS locations. 
...
# Get caret icons
fa_by_name("caret-up")
fa_by_name("caret-left")
fa_by_name("caret-down")
fa_by_name("caret-right")

up <- tmap_icons("caret-up.png")
left <- tmap_icons("caret-left.png")
down <- tmap_icons("caret-down.png")
right <- tmap_icons("caret-right.png")

mm_tmap_arrows <- tm_basemap(server = "CartoDB.VoyagerNoLabels")+
  tm_shape(poss_matches, bbox = exdrive_bbox_adj)+
  tm_lines(col = "tan", lwd = 10) +
  tm_shape(nearest_pts_ls_match, bbox = exdrive_bbox_adj)+
  tm_lines(col = "black")+
  ...
  tm_symbols(shape = "gyro_heading", size = 1, 
             shape.scale = tm_scale_intervals(
               style = "fixed",
               n = 5,
               #values = list(right, up, left, down, right),
               values = list(up, right, down, left, up),
               breaks = c(0, 45, 135, 225, 315, 360)),
             group = "original", group.control = "none", 
             shape.legend = tm_legend(title = "Vehicle Heading\n(0 Degrees = North)")) +
  tm_labels(text = "pt_label", xmod = 1.25, ymod = -0.5, options = opt_tm_labels(point.label = FALSE))+
  tm_dots(size = .5, fill = "white", group = "matched", group.control = "none", fill_alpha = 0.9)+ 
  tm_shape(nearest_pts_end_match, bbox = exdrive_bbox_adj)+
  tm_dots(size = 1, fill = "red", group = "matched", group.control = "none", fill_alpha = 0.9)

mm_tmap_arrows 
```
Example of the shortcomings of naively orthogonally projecting GPS points (white with underlying blue arrows showing the vehicle heading) onto the nearest road segment (red). Note the four nonsensically matched red points west of the intersection.
:::
...
@newsonHiddenMarkovMap2009 introduced a groundbreaking map-matching method. They used hidden Markov models (HMMs) to find the most likely route on the road system corresponding to a time-stamped set of GPS coordinates while accounting for noisy data and the configuration of the road network. However, this algorithm was not a real-time algorithm. Hidden Markov models are a common basis for many later map-matching algorithms [@sakiPracticalGuideOpenSource2022].
...
This chapter discusses software tools I developed for visualizing large-scale spatiotemporal transportation data and explains how insights based on the visualizations led to subsequent work.
...
This chapter covers my new method for determining a vehicle's location on a road segment. An application using this method is discussed.
