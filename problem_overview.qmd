...
library(tmap)
tmap_mode("plot")
library(emoGG) # HH not on CRAN, please include in comments how to install, like this: # remotes::install_github("dill/emoGG")
```
...
# Introduction and Background {#sec-background}

\hh{XXX The purpose of this first section is to answer a lot of the What? and Why? }
...
\mh{The Global Positioning System (GPS) is part of the Global Navigation Satellite System (GNSS), which provides accurate location and navigation information. For any GNSS (including GPS), the space segment is a constellation of satellites orbiting the Earth in a specified pattern. Typically, 6 to 10 of the satellites in the constellation are above the horizon at any time, but not all of the satellites may be available for use due to obstructions like tall buildings. The GPS control segment is made up of a network of monitoring and control stations that continually determine the orbital paths of the satellites in the space segment. The stations make short-term predictions of satellite orbital locations and transmit them back to the satellites so they can relay their positions to users, enabling real-time position information [@iliffeDatumsMapProjections2008].}
...
#| echo: false
#| layout-ncol: 2
#| fig-cap: "GPS Satellite Constellation Geometry Examples"
#| fig-subcap: 
#|  - "Poor Geometry"
#|  - "Good Geometry"
#| label: fig-gps
