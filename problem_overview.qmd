...
example_tmap <- function(basemap = "Esri.WorldImagery",
                         road_system,
                         bbox,
...
However, driving data collection devices can fail and return erroneous locations that do not match the vehicle's true locations. For example, recorded vehicle locations gathered from a trip through the streets of a city can fall nowhere near a roadway, although the vehicle spent the whole trip driving on public roads. Such erroneous data must be cleaned and corrected before they can be used in analyses. For example, to accurately calculate the traffic volume on a road segment, the number of vehicles driving on the road segment must be known. Vehicle locations recorded as being off the roadway will not be included in the analysis, obscuring the true traffic volume. For applications examining driver behavior, such as naturalistic driving studies, it is difficult to gain a full understanding of driver behavior without knowing where on the road system the study participants actually drove.

Several key factors are needed before vehicle trajectory data can be utilized to their full potential. Analysts need to visualize vehicle trajectories so they can assess the quality of the location data and explore how variables other than location (e.g., speed) change over the course of the drive. These changes can then be related to the geospatial trajectory. Driving data may include position quality measures, but these measures may not give an accurate picture of the true reliability of the recorded locations. This makes trajectory visualization all the more crucial.

Passenger cars are not the only type of vehicle for which trajectory visualizations are useful. For example, trajectory visualizations can also help to assess the performance of public transit systems utilizing the road system, such as buses. The paths taken by buses along their routes can be visualized, revealing bottlenecks or other areas of concern.

Errors in driving location data carry through to affect analyses done with the data. Thus, it is essential to project erroneously recorded driving location data onto the road system. The necessary level of the precision for the projection depends on the application. Some applications (such as route reconstruction) only require the knowledge of the sequence roads on which the vehicle traveled, while other applications depend on the precise location of the vehicle on the road segment. A wide variety of methods have been introduced to address this issue, with most methods heavily focused on selecting the road segment traversed by the vehicle at any recorded time point. After the road segment used by the vehicle has been selected, the vehicle's location on the segment may then be estimated.

In pursuit of our goals to create new statistical tools to visualize, process, and analyze large-scale geospatial transportation data, we use two primary telematics data sources for vehicle trajectories: naturalistic driving study (NDS) data and connected vehicle (CV) data. Both data sources make extensive use of location information acquired from the Global Positioning System (GPS). 

\mh{Accurate location information is also crucial for calculating performance metrics for work zones. Data sources such as CV data and probe vehicle data are very useful for this purpose. However, the traffic data used to calculate the performance metrics must first be geospatially matched to the work zones. Many US states provide information about their work zones through Work Zone Data Exchange (WZDx) feeds. These frequently updated data feeds give work zone information like the location, type of work, etc. However, different states provide the location information for their work zones in different ways. Some states provide the linestrings corresponding to the work zones, which makes it straightforward to identify the appropriate CV and/or probe vehicle data needed to calculate performance metrics for the work zone. Other states provide only the start and end points for their work zones, with no information about the shape of the road between them. This makes it challenging to match CV and/or probe vehicle data to the work zones. To solve this problem, I developed a method to find the roads corresponding to work zone start and end points and identify the shapes of the work zones as they follow the road system. It is then much simpler to calculate performance metrics for the work zones using CV and/or probe vehicle data.}

The organization of this work is as follows. We first discuss some essential background regarding GPS data in @sec-crs, @sec-gps, and @sec-dop. @sec-physics explains the physics of motion based on location data from vehicles. We then describe NDS data in @sec-nds and also describe CV data in @sec-cv. @sec-problemoverview provides an overview of the map-matching problem. Some map-matching algorithms and their drawbacks are discussed in @sec-MMalgos. Current solutions to the position determination problem in map-matching are provided in @sec-roadselection. A proposal for this work is found in @sec-proposal.
...
six different US states consented to have their driving monitored by an in-vehicle DAS equipped with various data collection channels, including video cameras, a global positioning system (GPS) unit, and accelerometers, among others [@campbellSHRP2Naturalistic2012]. The Longitudinal Research on Aging Drivers (LongROAD) study was a more recent NDS that focused on collecting data from participants aged 65 to 79 to better understand how medical, behavioral, environmental, and technological factors influence road safety as drivers age [@liLongitudinalResearchAging2017]. \mh{In a review of the literature as of 2022, @bayatDrivingAssessmentPreclinical2022 noted that several naturalistic driving studies involving older drivers had been conducted using the Driving Real-world In-Vehicle Evaluation System (DRIVES). These studies found differences in driving behavior between older adults with and without age-related cognitive decline. In particular, these studies found that older drivers with age-related cognitve decline tend to drive less aggressively and less often than older drivers not experiencing cognitive decline.}
...
\mh{The Institute for Transportation (InTrans) at Iowa State University currently purchases CV data from the third-party data provider StreetLight. (InTrans formerly purchased CV data from the third-party data provider Wejo Data Services, which has since ceased operations.) The original equipment manufacturers (OEMs) of connected vehicles use a onboard sensors to collect information about the vehicles throughout their journeys. The connected vehicles then wirelessly transmit these data to centralized cloud storage. The OEMs use these data to provide assistance to drivers and improve their driving experience. These data are also sold to data service companies after steps are taken to preserve customer and driver privacy. The data service companies wrangle, clean, and aggregate the CV data so they may be sold to research and academic institutions and transportation agencies for use in research and other applications [@kandiboinaIntersectionTechnologyTraffic2025].}

Connected vehicles collect a variety of data throughout their journeys, including vehicle location, speed, and heading. The connected vehicles directly provide trajectory information every 1-3 seconds throughout their journeys. (Data points are usually recorded every three seconds.) Each trip is uniquely identified by a unique JourneyID and each data point has a unique DatapointID. Other variables, such as acceleration, are derived from the data. @tbl-cvmetadata provides an overview of the available variables in the CV data, as described in the InTrans internal documentation. Similar to the NDS data, the most essential variables are the latitude/longitude, timestamp, speed, and heading. Unlike the NDS data, no measures of GPS quality are directly provided in the CV data.
...
## Vehicle Location Data Privacy Concerns

While vehicle location data, such as those collected by naturalistic driving studies and connected vehicles, are very useful for analysis applications, they also present serious data privacy concerns. A research report about vehicle geolocation data collection issues produced for the 119th Congress of the United States noted that the Federal Trade Commission (FTC) defines **geolocation data** as location data "sufficient to identify the street name and the name of the city or town." According to the same research report, the Department of Justice (DOJ) defines **precise geolocation data** as "data, whether real-time or historical, that identifies the physical location of an individual or a device with a precision of within 1,000 meters." Vehicle trajectory data can be considered personally identifiable information and can be used to identify drivers' visits to sensitive locations such as medical or reproductive health facilities, places of worship, or domestic abuse shelters [@souweidaneVehicleGeolocationData2025]. Even low-quality GPS data with a great deal of drift away from the road system can provide a location fix within 1,000 meters of the true location. The goal of this work is to provide vehicle location estimates significantly more accurate than this. Thus, it is essential to follow study policies for data protection when working with naturalistic driving data to protect participant privacy. The connected vehicle data used in this work were anonymized prior to being made available for analysis, but it is still crucial to keep data privacy in mind. 

## Work Zone Data from the Work Zone Data Exchange {#sec-wzdata}

\mh{The United States Department of Transportation's WZDx specification provides a standardized format for states to report road work zone locations, descriptions, and durations. More than 20 states (including Iowa, the focus of this research) currently utilize the WZDx format to publish work zone information. The WZDx specification does not require that states provide work zone location information in a specific format. Thus, some states (including Iowa) supply only the start and end points for their work zones. Other states supply linestrings reflecting the geometries for their work zones as they follow the topography of the road system between the work zone start and end points. A linestring geometry is essential for effective geospatial analysis and calculation of work zone performance metrics, so I developed a process to convert from work zone start and end points to linestrings.}

# Map-Matching Problem {#sec-MM}

## Overview {#sec-problemoverview}

As we have worked with driving data, particularly the NDS data from the UNMC study, we have noticed that GPS location data can drift away from the road system, even when the DOP coefficients for the data points fall within the Excellent to Good ranges from the scale in @tbl-dop. The blue points in @fig-gps-drift are the original points returned by the GPS unit installed in a vehicle used in a NDS, and the PDOP values for these points range from 1.63 to 2.36. Despite their low PDOP values, these points do not fall on the road system and thus are not high quality GPS location data for our applications. This situation also illustrates the need for analysts to explore and visualize driving trajectory data before performing any analyses. GPS errors like these make geospatial analysis of the data much more challenging. 
...
Regardless of the intended application, all map-matching algorithms require a geospatial representation of the road system where the vehicle trajectories in question may be assumed to have occurred. For NDS and CV driving data, it is reasonable to expect the vast majority of driving will take place on public roads. The scale and quality of the road system representation used in the map-matching process affects the potential accuracy of corrected vehicle trajectories that may be achieved. The quality of the GPS data available also affects the quality of results from map-matching algorithms. @millard-ballMapmatchingPoorqualityGPS2019 considered GPS data to be poor quality for map-matching if the data collection interval was sparse (e.g., 30 seconds or more between observations), the spatial measurements were off by more than 50 m, or the vehicle was recorded as having made unrealistic movements (e.g., appearing to teleport). They then created a map-matching algorithm designed to mitigate such GPS data quality issues in road selection.
...
gps_drift_tmap <- tm_basemap(server = "Esri.WorldImagery") +
  tm_shape(poss_matches, bbox = exdrive_bbox_adj)+
  tm_lines(col = "tan", lwd = 10)+
  tm_shape(exdrive_plot, bbox = exdrive_bbox_adj)+
...
mm_tmap_heading <- tm_basemap(server = "Esri.WorldImagery")+
  tm_shape(poss_matches, bbox = exdrive_bbox_adj)+
  tm_lines(col = "tan", lwd = 10) +
...
Due to their poor performance, geometric map-matching methods have fallen out of favor in the literature. A vast array of more sophisticated map-matching algorithms employing a plethora of estimation techniques have been proposed. The work by @newsonHiddenMarkovMap2009 is widely regarded as seminal. They used hidden Markov models (HMMs) to find the most likely route on the road system corresponding to a time-stamped set of GPS coordinates while accounting for noisy data and the configuration of the road network. Hidden Markov models have since become a basis for many later map-matching algorithms [@sakiPracticalGuideOpenSource2022]. \mh{Despite their overall good performance, map-matching methods based on HMMs are oversimplifications of reality (as are all models). In particular, typical map-matching methods based on HMMs consider the vehicle position at time $t$ to only depend on the vehicle position at time $t-1$, as mentioned by @songMapMatchingAlgorithm2023. @songMapMatchingAlgorithm2023 noted that map-matching methods using higher-order HMMs that consider times earlier than $t-1$ have been proposed, but the computational complexity of these methods increases rapidly, preventing them from going beyond second-order models.} However, even highly complex map-matching methods can still experience issues with incorrect road selection, such as those illustrated in @fig-naive-map-matching and @fig-map-matching-heading.
...
Despite its widespread use and continued popularity, orthogonal projection is not the optimal way to solve the position determination problem in all cases. @greenfeldMatchingGPSObservations2002 observed that position errors and errors in the representation of the road system subsequently lead to errors in vehicle locations projected onto road segments. Thus, the actual location of the vehicle and its projected location on the road system may disagree. Orthogonal projection may also lead to a loss or misspecification of the temporal relationship between the recorded points. Alternative strategies to orthogonal projection have been proposed.
...
For $m \in 1, ..., k-1$, define $\symbfit{Z}^{(m-)} = \left(Z_j^{(m-1)}\right)_{j=1}^{n_m}$ and $\symbfit{Z^{(1:m)}} = \left\{\symbfit{Z}^{(1)}, ..., \symbfit{Z}^{(m)}\right\}$. These vectors indicate which records refer to the same entity, and thus are parameters of interest in the record linkage problem.

Recall that a record for an entity can appear at most once in each data file. Thus, all of the records corresponding to a specific entity may be represented as a chain of links from the most recent record back to the earliest record. Records are linked in the order they appeared. Thus, all records corresponding to a specific entity may be identified by starting at one link in the chain and traversing the chain forwards and backwards. New links are added to the chain and older links may be updated as new data files arrive. An implementation of @taylorFastBayesianRecord2024's Bayesian streaming record linkage model is available in the R package bstrl [@betancourtBstrlBayesianStreaming2022].
...
Acquisition of ground truth vehicle trajectory data is another challenge in developing and validating map-matching algorithms. Many authors collect their own vehicle trajectories without sharing them or use data unavailable to the general public. However, there are some notable exceptions to this practice. @newsonHiddenMarkovMap2009 provided one example vehicle trajectory from a route in Seattle, Washington, USA, along with the ground truth sequence of road segments traversed by the vehicle. @kubickaDatasetTestingTraining2015 provided one hundred example vehicle trajectories from various locations around the world along with a map-matched route for each trajectory. However, the true sequence of road segments traveled in each of these trajectories were unknown, and thus the matched routes represented experts' best ideas for the correct sequence of road segments. The sets of example trajectories from Newson and Krumm and Kubička et al. focused more on the determination of the road segment on which the vehicle was traveling at each time point as opposed to the location of the vehicle on the selected link.

Another important aim of this work is to collect new driving trajectories and make them available as a public dataset. Each trajectory will be recorded by three different GPS receivers of varying precisional capability. We will produce a dataset with point-by-point ground truth road segment location information. This dataset will play a critical role in the development of the position determination algorithm, and it will be made available for others to use in their own work. Therefore, barriers to entry in the map-matching space will be reduced.

Thus, we can utilize highly accurate estimates for the road on which a vehicle is driving and combine them with a more precise position determination method than is achievable from simple orthogonal projection onto the selected road segment. The separation from the map-matching algorithm for road selection will allow our position determination algorithm to be used with any existing or yet-to-be-proposed map-matching algorithm for road selection. Our work will also include novel ways to visualize large-scale spatio-temporal transportation data with DrivePlotR and unlock new insights from applications. These new insights will not be possible without new ways to visualize large-scale spatio-temporal transportation data and an improved method to determine the positions of vehicles on the road system after matching. We will also provide data used in the development of our position determination method so others may build on the work.
...
This chapter discusses software tools developed for visualizing large-scale spatio-temporal transportation data and explains how insights based on the visualizations led to subsequent work. \mh{This chapter also discusses a method to match work zone locations in Iowa to the road system.}
